---
name: new-or-review
description: 写博客文章、审阅已有文章、降低 AI 味或根据素材改稿时使用。先读作者认知框架，决定文章推进方式，再调用具体经历做证据；审稿时用内置脚本辅助发现模板化表达、证据缺失和节奏问题，但最终判断以作者画像和人工审稿为准。
---

# 博客写作与审稿

这个 skill 负责两类任务：

- **新建**：从主题到终稿，走完整的流程
- **审阅**：对现有文章做技术审校、风格审校和 AI 痕迹 / author-fit 检查

## 核心联动机制

### 与 author-profile 的联动（最重要）

写作前**必须先读认知框架**，理解底层假设，再决定文章推进方式。

**联动流程**：

1. 读 `.claude/skills/author-profile/PROFILE.md` 认知框架部分
2. 根据文章主题，确定哪个认知维度最相关：
   - 技术学习文章 → 知识观（文章要展示认知过程）
   - 系统设计文章 → 工程观（文章要展示 trade-off）
   - 经历复盘文章 → 成长观（文章要展示认知螺旋）
   - 所有文章 → 表达观（文章要有锚点，允许留白）
3. 根据认知框架决定文章结构：
   - 知识观 → 结构是”误解 → 撞墙 → 修正”，不是”定义 → 特性 → 应用”
   - 表达观 → 开头用问题或场景切入，结尾允许没讲满
   - 工程观 → 系统设计要展示 trade-off 思考过程
   - 成长观 → 翻车经历要保留，不能删成完美阶梯
4. 调用具体经历做证据 → 每段经历要关联认知转折
5. 写作完成后检查：是否体现了认知框架？还是只在引用事迹？

**关键原则**：

- 引用事迹 ≠ 理解认知框架
- “他在 OceanBase 做过可观测平台” → 只引用事迹
- “他在 OceanBase 因为需求边界模糊翻车，此后对'需求边界澄清'有体感” → 理解认知框架

### 与 humanizer-zh 的联动

改写时先读表达观，理解为什么不喜欢某些表达：

- 删掉”回头看” → 因为成长感不该硬提醒
- 删掉”本质上” → 因为不该假装全知
- 保留作者语气 → 因为情绪可以有，但不能表演

## 目录

- `references/author-constraints.md`
  - 作者画像、知识边界、写作硬约束、稳定表达偏好

- `references/new-post-workflow.md`
  - 新建文章的 `Stage 0 -> Stage 5`

- `references/review-and-revise.md`
  - 技术事实检查、风格检查、内置脚本信号、revision plan、停止条件

- `references/article-types.md`
  - 技术深潜、系统设计、读书笔记、实践复盘的骨架

- `references/anti-patterns.md`
  - 高频 AI 痕迹、模板化过渡、禁用词和改写方向

- `references/style-extraction-method.md`
  - 如何从样本文本里提取文风，如何练习模仿，而不是只学表面句式

- `references/xiaolin-style.md`
  - 基于 `samples/xiaolin.md` 和 `samples/xiaolin2.md` 提取的小林文风样本卡片，以及“借骨架不借人格”的适配规则

- `references/xiaolin-adaptation-template.md`
  - “借小林骨架，但仍然是作者本人”的提纲模板、写作提示词骨架和审稿自检清单

## 内置脚本与可选样本

- `scripts/review_signals.py`
  - 轻量程序化审稿脚本；相对当前 skill 目录解析
  - 只负责输出可疑信号，不替代人工判断，不负责代写终稿

- `samples/xiaolin.md`
  - 小林样本文本 1；用于重新提取或校准文风卡片

- `samples/xiaolin2.md`
  - 小林样本文本 2；用于重新提取或校准文风卡片

可选样本缺失时：

- 不得假装已经读取原始样本
- 必须明确报告缺失了哪个相对路径
- 可以继续使用 `references/` 里的沉淀卡片做有限分析，但不能冒充做了完整样本校准

## 使用顺序

### 新建文章

1. 先读 `.claude/skills/author-profile/PROFILE.md`
2. 再读 `_not_publish/经历与思考素材库.md`
3. 如果主题涉及具体项目，再读相关经历文件或 `_not_publish/jianli.md`
4. 再读 `references/author-constraints.md`
5. 再读 `references/new-post-workflow.md`
6. 根据文章类型按需读 `references/article-types.md`
7. 如果用户明确要求模仿某种文风，读 `references/style-extraction-method.md` 和对应样本卡片
8. 如果需要重新提取或校准小林文风卡片，再读 `samples/xiaolin.md` 和 `samples/xiaolin2.md`
9. 如果是“像小林，但内容仍然是我自己的”，再读 `references/xiaolin-adaptation-template.md`
10. 首稿完成后读 `references/review-and-revise.md`
11. 如果有明显套话或模板味，再补读 `references/anti-patterns.md`
12. 如果本轮暴露了新的稳定偏好或素材，提出回写 `author-profile` 的建议；经用户确认后再回写

### 审阅现有文章

1. 先读 `.claude/skills/author-profile/PROFILE.md`
2. 必要时读 `_not_publish/经历与思考素材库.md`
3. 再读 `references/author-constraints.md`
4. 再读 `references/review-and-revise.md`
5. 若问题集中在模板味、AI 味、结尾过度收束，再读 `references/anti-patterns.md`
6. 如果用户要求“像某个作者/样本”，再读对应样本卡片
7. 如果用户要求“像小林，但还是我自己”，再读 `references/xiaolin-adaptation-template.md`
8. 如果用户真实改稿暴露了新的稳定偏好，提出回写 `author-profile` 的建议；经用户确认后再回写

### 提取或学习文风

1. 先读 `references/style-extraction-method.md`
2. 如果要重新提取或更新卡片，先读原始样本，例如 `samples/xiaolin.md` 和 `samples/xiaolin2.md`
3. 再读已有样本卡片，例如 `references/xiaolin-style.md`
4. 输出时区分：
   - `可观察特征`
   - `推测性偏好`
   - `可训练动作`
5. 不把样本的主题、口头禅、格式习惯误当成全部文风

## 不可违反的规则

1. 不让 AI 一步到位写完整篇文章。
2. 内置脚本只负责提供可疑信号，不负责代写终稿，也不替代审稿判断。
3. 修订时只改命中的段落，不整篇重写。
4. 自动回环最多 2 到 3 轮，之后交给人工裁决。
5. 只要存在认知边界、材料缺口、叙述方式不确定，先访谈，不硬写。
5.1. 任何时候，只要你对写出来的东西有一丝「这可能不是作者真正相信的」的感觉，必须停下来问，不要继续。
6. 篇级句法限额要当硬约束处理：
   - “不是……而是……”最多 1 次（默认目标 0 次）
   - “回头看”最多 1 次（默认目标 0 次）
   - “后来才（发现/意识到/明白）”最多 2 次
   - “按我现在的理解””我更愿意把……理解成”最多 1 次
   超出即按模板化写作处理并优先改掉。

## 程序化审稿脚本

脚本路径：`scripts/review_signals.py`（相对当前 skill 目录）

它是轻量辅助工具，不是独立判定系统。优先用于审阅已有文章、首稿后的 review、以及用户明确要求“降 AI 味”“查模板感”的场景。

脚本只负责：

- 标记可能的 AI 痕迹、模板化表达、节奏问题和硬证据缺失
- 给出段落级位置、命中词和修改方向
- 帮助 review 更快定位问题

脚本不负责：

- 定义本 skill 的写作 prompt
- 直接产出最终文章
- 判断整篇文章是否像作者本人
- 决定整篇文章的最终取舍

使用方式：

```bash
py .claude/skills/new-or-review/scripts/review_signals.py path/to/article.md
```

如果环境里的 `python` 命令可用，也可以把 `py` 换成 `python`。

把脚本输出当成 `signals`，再结合 `author-profile`、文章材料和人工审稿判断生成 revision plan。脚本没有命中的地方不代表没有问题；脚本命中的地方也不代表必须修改。

## 输出原则

- 首先报告证据和判断，不先端漂亮结论
- 允许局部不确定，但不能把不确定写成事实
- 优先保留作者自己的材料、火气、犹豫和认知来路
- 需要回访作者时，明确列出少量关键问题，不铺满
