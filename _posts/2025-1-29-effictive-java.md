...
这些工具应该接入 CI 流水线——在《持续交付》中学到的部署流水线里，静态分析属于提交阶段，和编译、单元测试一起跑。红了就不让合入。
...
在《JVM 虚拟机》中学到的 jps/jstat/jmap/jstack/JFR 这套工具链，和 Effective Java 的并发、对象创建章节是直接对应的。当时只记了工具名，现在才理解它们各自在观测什么。
