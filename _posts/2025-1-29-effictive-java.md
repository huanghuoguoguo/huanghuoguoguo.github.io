...
我现在更愿意承认，是自己当时还没被足够多的 bug 教训过。等这些教训攒起来以后，很多条目就不再只是“知道有这回事”，而会慢慢变成条件反射。
...
我后来觉得，继承最麻烦的地方在于子类和父类的实现细节缠得太深。父类的 `addAll()` 内部调不调 `add()`，这是实现细节，不是契约。一旦父类改了实现，子类就很容易跟着炸。组合至少能把这层耦合隔开。

---

## 第 9 章（条目 61）× 第 2 章（条目 6）：基本类型优于装箱类型

这一条我单独拎出来讲，纯粹是因为我在项目里真的吃过亏。
