...
这篇文章将深入复盘这次 RAG 架构迁移的动机、设计思路、技术选型和踩过的坑，最后聊聊前沿 RAG 技术和未来方向。

---

## 1. 从一个“劝退”的故事说起

前段时间，一位负责维护 LangBot RAG 插件的用户在群里说，他正尝试为 LangBot 编写一个外置知识库插件，结果写着写着发现不对劲：

“我发现向量能力、Parser 能力、LLM 调用能力，如果全都要插件自己去维护和对接的话，开发的心智成本实在太高了！”

他的吐槽一针见血地戳中了旧架构的痛点：底层缺乏统一的基础设施支持，导致插件开发者必须“重复造轮子”。这正是促使我们下定决心做 **RAG 架构统一** 的核心诱因。

至于 RAG 是什么，相信关注 AI 的同学已经在各种文章里看腻了（我在之前的文章里也详细讲过），这里就不再复读 ABC 了。我们跳过背景介绍，直接进入这场关乎“架构正义”的重构。
...
**1. 内置 RAG 策略太死。** 只有一种分块策略（固定大小递归切分），没有语义切分、没有 Parent-Child 分块、没有 QA 对索引、没有查询改写。想加新策略就得改核心代码，发版周期长，社区贡献门槛高。

**2. 两套知识库并存增加了维护成本。** 内置和外置的数据模型不同（`KnowledgeBase` vs `ExternalKnowledgeBase`）、API 不同、前端组件不同。每次改一个功能要改两遍，容易漏。

**3. 外置知识库缺少框架层支持。** 对接外部 RAG 引擎（如 RAGFlow、Dify）的插件本质上是一个转发层，这本身没问题。问题在于 LangBot 内部对内置和外置知识库没有一视同仁——判断逻辑分散，插件需要自己处理数据一致性，开发门槛较高。与其让每个插件各自解决这些问题，不如由 LangBot 提供统一的基础设施能力（文件上传、索引管理等）。

核心矛盾是：**LangBot 想同时满足"开箱即用"和"深度可定制"，但内置实现做不到后者，外置方案缺少框架层支持做不好前者。**
...
LangBot 支持多种向量数据库（Chroma、Qdrant、Milvus、pgvector、SeekDB），它们的能力差异很大。比如 Qdrant 和 Milvus 支持混合检索（稀疏+稠密），而有的向量库只支持纯向量检索。RAG 插件需要知道当前配置的向量库支持什么，才能选择最优策略。
...
这次迁移真正完成的事，是**把 RAG 从固定功能变成了可插拔的能力。**
...
架构迁移只是起点。有了插件化的基座，后续的 Parser 解耦、TOC 辅助检索、长期记忆系统都可以作为独立插件渐进地加入，每一步都不需要动核心代码。这大概就是"好架构"的意义——**让未来继续演进的成本尽可能低。**

最后补一张上线以后的反馈图嘿嘿：

![群聊截图](/images/LangRag-qq.jpg)
