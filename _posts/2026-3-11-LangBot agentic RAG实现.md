...
**Naive RAG 能用，但它把检索时机和检索方式都提前写死了。** Agent 明明有判断能力，我们却没给它判断的机会。一个有 tool calling 能力的 Agent 完全可以自己判断"这个问题需不需要查资料"，自己决定"去哪个库查、用什么 query 查"，甚至在第一次查的结果不够好时换个 query 再查一次。
...
所以最终更合理的做法，还是让用户根据自己的场景选择。这也是为什么我们把 Agentic RAG 做成可选插件，而不是替换掉 Naive 模式——**框架把选择权留给用户，就像我们把判断权留给 Agent 一样。**
